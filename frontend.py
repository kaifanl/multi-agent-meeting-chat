...
from analysis_agent import MeetingAnalyzer
from transcript_agent import AudioProcessingAgent
from qwen_api import APILLMResponse

def analyze_meeting_text_v2(content):
    analyzer = MeetingAnalyzer(api_key_path='qwen.key')
...
def transcript_meeting_text(audio_file_path):
    ...
    result = agent.process_audio(audio_file_path)
...
def chunk_text(text, chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP):
    ...
    while start < len(text):
        ...
        if start + chunk_overlap >= len(text) and end < len(text):
            chunks.append(text[len(text)-(chunk_size-chunk_overlap):])
            break
...
def get_embedding(text_chunk):
    raise NotImplementedError("请提供真实的embedding模型实现来替换此函数。")
...
def process_meeting_file(meeting_name, uploaded_file_obj):
    ...
    rag_data_filename = f"{safe_filename_base}_rag.json"
    rag_data_file_path = os.path.join(RAG_DATA_DIR, rag_data_filename)
    ...
    st.session_state.uploaded_meetings[meeting_name]['rag_data_file_path'] = rag_data_file_path
    ...
    try:
        ...
        summary_content = analyze_meeting_text_v2(transcript_text, meeting_name=meeting_name)
...
def get_chat_response_rag(user_query, selected_meeting_names):
            ...
            if os.path.exists(rag_path):
                try:
                    with open(rag_path, "r", encoding="utf-8") as f:
                        ...
                        all_chunks_data.extend(meeting_rag_data)
    ...
    if chunk_embeddings.ndim == 1:
        if chunk_embeddings.shape[0] == query_embedding.shape[1]:
    ...
    num_chunks_to_consider = min(len(similarities), TOP_K_CHUNKS * 2)
...
if st.session_state.get('show_summary_for_meeting_name'):
    ...
    processed_meetings_options = [
        ...
        if data.get('status') == 'processed' and data.get('rag_data_file_path')
    ]
